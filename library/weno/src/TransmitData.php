...
class TransmitData
{
    ...
    {
        $sql = "SELECT p.date_Added, p.date_Modified, p.drug, ew.rxcui_drug_coded as drug_id, p.dosage, p.refills, p.quantity, p.note,".
               "ew.strength, ew.route, ew.potency_unit_code, ew.drug_db_code_qualifier,ew.dea_schedule FROM prescriptions AS p ".
               "RIGHT JOIN erx_weno_drugs AS ew ON p.drug = ew.full_name WHERE p.id = ?";
